...
export const Users = () => {
  ...
  useEffect(() => {
    if (!filter.trim()) return; // no debounce call for empty search
  ...
  const fetchUsers = (searchTerm) => {
    try {
      ...
      axios
        .get(
          `https://pay-money.onrender.com/api/v1/user/bulk?filter=${searchTerm}`,
          {
            headers: {
        ...
        .then((response) => {
          const filteredUser = response.data.users.filter(
            (user) => user.User_name !== decoded_user.User_name
          );
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-sm">
      <div className="font-bold text-2xl text-gray-800 mb-6">Users</div>
      ...
      <div className="space-y-4">
        {users.length > 0 ? (
          users
            .filter((user) => logged_In_User !== user.User_name)
